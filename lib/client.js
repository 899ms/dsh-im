...
function rpcEndpoint(channel6) {
  if (!/^\/[A-Za-z0-9._~-]+$/.test(channel6)) throw new TypeError("Invalid IM RPC channel");
  return `dsh-im${channel6}`;
}
function callManagementRpc(connection, channel6, method, payload, signal) {
  return connection.rpc.call("/api", rpcEndpoint(channel6), { method, payload }, signal);
}
...
function CredentialBindingPanel({
  ...
  return h2(
    ...
    h2("h3", { id: headingId, className: "dim-credentialTitle" }, `\u624B\u52A8\u63A5\u5165${channel6}\u673A\u5668\u4EBA`),
    h2(
      "form",
      {
...
function createTokenChannelApi(channel6, connectionSummary, {
  ...
  const unwrapRpcResult16 = (result) => {
    if (!isRecord2(result) || typeof result.ok !== "boolean") {
      throw new Error(`${channel6} \u670D\u52A1\u8FD4\u56DE\u4E86\u65E0\u6CD5\u8BC6\u522B\u7684\u54CD\u5E94`);
    }
    if (!result.ok) {
      const error = new Error(text2(result.error?.message, `${channel6} \u64CD\u4F5C\u5931\u8D25`));
      error.code = text2(result.error?.code, `${channel6.toUpperCase()}_RPC_ERROR`, 80);
      throw error;
    }
    return result.value;
  };
  const normalizeBot8 = (value) => {
    ...
    return {
      ...
      bot: {
        ...
        name: text2(value.bot?.name, `${channel6}\u673A\u5668\u4EBA`, 100),
      ...
      health: {
        summary: text2(
          ...
          connected ? `${channel6}${connectionSummary}\u8FD0\u884C\u6B63\u5E38` : `${channel6}\u8FDE\u63A5\u5C1A\u672A\u5C31\u7EEA`
        ),
        lastCheckedAt: timestamp2(value.health?.lastCheckedAt)
      },
      lastMessageError: normalizeLastMessageError(value.lastMessageError),
      error: isRecord2(value.error) ? {
        code: text2(value.error.code, `${channel6.toUpperCase()}_ACCOUNT_ERROR`, 80),
        message: text2(value.error.message, `${channel6}\u8FDE\u63A5\u5C1A\u672A\u5C31\u7EEA`)
      } : null,
      ...isRecord2(extension) ? extension : {}
    };
  };
  const normalizeSnapshot12 = (value) => {
    const source = isRecord2(value?.snapshot) ? value.snapshot : value;
    if (!isRecord2(source) || !Array.isArray(source.bots)) {
      throw new Error(`${channel6} \u670D\u52A1\u6CA1\u6709\u8FD4\u56DE\u6709\u6548\u7684\u673A\u5668\u4EBA\u5217\u8868`);
    }
  ...
  const presentError16 = (error) => ({
    code: text2(error?.code, `${channel6.toUpperCase()}_ERROR`, 80),
    message: text2(error?.message, `${channel6}\u64CD\u4F5C\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5`)
  });
  return Object.freeze({ unwrapRpcResult: unwrapRpcResult16, normalizeSnapshot: normalizeSnapshot12, presentError: presentError16 });
}
...
function createTokenChannelSettings(definition) {
  const {
    ...
    credentialAriaLabel = `\u4F7F\u7528 Bot Token \u63A5\u5165 ${channel6} \u673A\u5668\u4EBA`,
  ...
  function SettingsTab({ rpcCall }) {
    ...
    const invoke = React14.useCallback(async (endpoint, payload = {}, signal) => {
      if (typeof rpcCall !== "function") throw new TypeError(`${channel6} \u8BBE\u7F6E\u9875\u7F3A\u5C11 RPC \u8FDE\u63A5`);
      return api6.unwrapRpcResult(await rpcCall(endpoint, payload, signal));
    }, [rpcCall]);
    const loadStatus = React14.useCallback(async ({ signal, silent = false } = {}) => {
      ...
      try {
        const snapshot = api6.normalizeSnapshot(await invoke(endpoints.status, {}, signal));
    ...
    const bindCredentials = React14.useCallback(async (values) => {
      ...
      setCredentialError(null);
      try {
        const snapshot = api6.normalizeSnapshot(await invoke(
        ...
        if (mounted.current) setCredentialError(api6.presentError(error));
      } finally {
        const shouldRefresh = workspaceFence.endMutation();
    ...
    const botAction = React14.useCallback(async (account, operation, endpoint, payload) => {
      ...
      try {
        ...
        const snapshot = api6.normalizeSnapshot(value);
    ...
    return h2(ModelCatalogContext.Provider, {
      ...
      model.phase === "loading" ? h2("div", {
        ...
        "aria-busy": "true"
      }, h2("div", { className: "ddt-spinner dim-spinner" }), `\u6B63\u5728\u8BFB\u53D6 ${channel6} \u673A\u5668\u4EBA\u72B6\u6001\u2026`) : model.phase === "error" ? h2(
        "div",
        { className: "ddt-card dim-surfaceCard" },
        h2(
          "div",
          { className: "ddt-inlineError dim-inlineError" },
          h2("h3", null, `\u65E0\u6CD5\u8BFB\u53D6 ${channel6} \u673A\u5668\u4EBA\u72B6\u6001`),
          ...
          h2(
            ...
            h2(
              ...
              h2(
                ...
                { className: "ddt-stateLabel dim-stateLabel" },
                h2("span", { className: "ddt-dot dim-stateDot" }),
                h2("span", null, `\u5C1A\u672A\u63A5\u5165 ${channel6} \u673A\u5668\u4EBA`)
              ),
...
var api5 = createTokenChannelApi("Matrix", " CS API \u957F\u8F6E\u8BE2");
var unwrapRpcResult12 = api5.unwrapRpcResult;
var normalizeSnapshot11 = api5.normalizeSnapshot;
var presentError12 = api5.presentError;
...
function MatrixCredentialPanel({ busy, error, onSubmit, onCancel }) {
  const [homeserver, setHomeserver] = React26.useState("");
  const [accessToken, setAccessToken] = React26.useState("");
  const [userId, setUserId] = React26.useState("");
  const [password, setPassword] = React26.useState("");
  const headingId = React26.useId();
...
var channel5 = createTokenChannelSettings({
  channel: "Matrix",
...
var MatrixSettingsTab = channel5.SettingsTab;
var MatrixAccountCard = channel5.AccountCard;
...
function AccessPolicySettingsPage({ channel: channel6, account, rpcCall, onSaved }) {
  const definition = ACCESS_CHANNEL_DEFINITIONS[channel6];
  ...
  const save = async (event) => {
    ...
    try {
      ...
      const value = unwrapRpcResult13(await rpcCall(ACCESS_POLICY_ENDPOINT, {
        botId: account.botId,
        policy
      }));
...
function botSettingsTabsForChannel(channel6) {
  return channel6 === "feishu" ? FEISHU_BOT_SETTINGS_TABS : BOT_SETTINGS_TABS;
}
...
function TargetForm({
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setError(presentError13(caught, "\u76EE\u6807\u4FDD\u5B58\u5931\u8D25\uFF0C\u8BF7\u68C0\u67E5\u540E\u91CD\u8BD5\u3002"));
    }
  };
  const testTarget = async () => {
    ...
    try {
      ...
      setTestState({ tone: "error", message: presentError13(caught, "\u6D4B\u8BD5\u53D1\u9001\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002") });
    } finally {
      setTesting(false);
    }
  };
...
function TargetRow({ definition, target, botId, connected, rpcCall, onChanged, onEdit }) {
  ...
  const testTarget = async () => {
    ...
    try {
      ...
      setTestState({ tone: "error", message: presentError13(error, "\u6D4B\u8BD5\u53D1\u9001\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002") });
    } finally {
      setAction(null);
    }
  };
  const copyPair = async () => {
    ...
    try {
      ...
      setCopyState(presentError13(error, "\u590D\u5236\u5931\u8D25\u3002"));
    }
  };
  const deleteTarget = async () => {
    ...
    try {
      ...
      setTestState({ tone: "error", message: presentError13(error, "\u5220\u9664\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002") });
      setAction(null);
  ...
  const setSessionSync = async (enabled) => {
    ...
    try {
      ...
      setSyncFeedback({
        tone: "error",
        message: error?.code === "session-sync-unavailable" ? "\u4F1A\u8BDD\u540C\u6B65\u4EC5\u652F\u6301\u5DF2\u7ECF\u804A\u8FC7\u3001\u5DF2\u5EFA\u7ACB\u5F53\u524D\u4F1A\u8BDD\u4E14\u4F7F\u7528\u5F53\u524D Host Harness \u7684\u79C1\u804A\u76EE\u6807\u3002" : presentError13(error, "\u4F1A\u8BDD\u540C\u6B65\u8BBE\u7F6E\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002")
      });
    } finally {
      setAction(null);
    }
...
function DeliveryTargetSettingsPage({
  ...
  const definition = CHANNEL_DEFINITIONS[channel6];
  ...
  const loadTargets = React29.useCallback(async ({ signal, silent = false } = {}) => {
    ...
    try {
      ...
      setError(presentError13(caught, "\u65E0\u6CD5\u8BFB\u53D6\u6295\u9012\u76EE\u6807\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002"));
      setPhase("error");
    }
  ...
  const loadSuggestions = React29.useCallback(async () => {
    ...
    try {
      ...
      setSuggestionError(presentError13(caught, "\u65E0\u6CD5\u8BFB\u53D6\u5DF2\u804A\u4F1A\u8BDD\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002"));
  ...
  const copyBotId = async () => {
    ...
    try {
      ...
      setBotCopyState(presentError13(caught, "\u590D\u5236\u5931\u8D25\u3002"));
    }
  };
  const settingsTabs = botSettingsTabsForChannel(channel6);
  ...
  return h2(
    ...
    h2(
      ...
      }) : activeTab.id === "group" && channel6 === "feishu" ? h2(FeishuGroupSettingsPage, {
        account,
        rpcCall: accessRpcCall
      }) : h2(
        React29.Fragment,
...
function GlobalSettingsPanel({ rpcCall }) {
  ...
  const loadSettings = React30.useCallback(async ({ signal } = {}) => {
    ...
    try {
      ...
      if (signal?.aborted || caught?.name === "AbortError" || !mounted.current) return;
      setLoadError(presentError14(caught, "\u65E0\u6CD5\u8BFB\u53D6\u901A\u7528\u8BBE\u7F6E\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002"));
      setPhase("error");
    }
  }, [invoke]);
  React30.useEffect(() => {
  ...
  const commitTtl = async () => {
    ...
    try {
      ...
      if (mounted.current) {
        setSaveError(presentError14(caught, "\u8BBE\u7F6E\u4FDD\u5B58\u5931\u8D25\uFF0C\u8BF7\u7A0D\u540E\u91CD\u8BD5\u3002"));
      }
    } finally {
...
}).map(([channel6, labels]) => [channel6, Object.freeze(labels)])));
function parseSessionChannelTitle(title) {
  if (typeof title !== "string") return null;
  for (const [channel6, labels] of Object.entries(SESSION_CHANNEL_LABELS)) {
    for (const label of labels) {
      const prefix = `${label} \xB7 `;
      if (title.startsWith(prefix) && title.slice(prefix.length).trim()) {
        return { channel: channel6, title: title.slice(prefix.length) };
      }
    }
  }
  return null;
...
function logoUri(document2, channel6) {
  const [Glyph, color] = GLYPHS[channel6];
  const render = (node) => {
...
function createInstallation(document2) {
  ...
  const uris = Object.fromEntries(Object.keys(GLYPHS).map((channel6) => [channel6, logoUri(document2, channel6)]));
  style.textContent = `
${MARKED}[${TITLE_ATTR}] {
  position: relative;
...
${Object.entries(uris).map(([channel6, uri]) => `[${CHANNEL_ATTR}="${channel6}"] { --dsh-im-session-logo: url("${uri}"); }`).join("\n")}
`;
  ...
  for (const [channel6, uri] of Object.entries(uris)) {
...
function UpdatePanel({ rpcCall, clientVersion, onStatus }) {
  ...
  React31.useEffect(() => {
    ...
    void invoke("update.status", {}, controller.signal).then((next) => {
      ...
      if (!controller.signal.aborted) setError(presentError15(cause));
    }).finally(() => {
  ...
  React31.useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (!pendingController.signal.aborted) setError(presentError15(cause));
      } finally {
  ...
  const readSnapshot = async (endpoint) => {
    ...
    try {
      ...
      if (!controller.signal.aborted && mounted.current) setError(presentError15(cause));
    } finally {
      if (!controller.signal.aborted && mounted.current) {
        busy.current = false;
        setAction(null);
...
function ChannelLogo({ channel: channel6 }) {
  if (channel6 === "weixin") return h2(WeixinLogo);
  if (channel6 === "feishu") return h2(FeishuLogo);
  if (channel6 === "dingtalk") return h2(DingtalkLogo);
  if (channel6 === "wecom") return h2(WecomLogo);
  if (channel6 === "wecomApp") return h2(WecomAppLogo);
  if (channel6 === "qq") return h2(QqLogo);
  if (channel6 === "slack") return h2(SlackLogo);
  if (channel6 === "telegram") return h2(TelegramLogo);
  if (channel6 === "discord") return h2(DiscordLogo);
  if (channel6 === "whatsapp") return h2(WhatsappLogo);
  if (channel6 === "imessage") return h2(IMessageLogo);
  if (channel6 === "matrix") return h2(MatrixLogo);
...
function IMSettingsTab({
  ...
  const active = CHANNELS.find((channel6) => channel6.id === selected) ?? CHANNELS[0];
    ...
    h2(
      ...
      h2(
        ...
        h2(
          ...
          CHANNELS.map((channel6) => h2(
            "button",
            {
              key: channel6.id,
              type: "button",
              role: "tab",
              id: `dim-tab-${channel6.id}`,
              className: "dim-channel",
              "aria-selected": !globalSettingsSelected && channel6.id === active.id,
              "aria-controls": `dim-panel-${channel6.id}`,
              onClick: () => {
                setSelected(channel6.id);
                setDeliverySettings(null);
              }
            },
            h2(ChannelLogo, { channel: channel6.id }),
            h2(
              "span",
              { className: "dim-channelCopy" },
              h2("strong", null, channel6.label),
              channel6.note ? h2("small", { className: "dim-channelNote" }, channel6.note) : null
            )
          ))
        ),
